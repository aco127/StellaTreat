...
public class Week1CardsCheckStellaLite {
	WebDriver driver;
	Properties prop;

	public Week1CardsCheckStellaLite(WebDriver driver) {
 this.driver = driver; PageFactory.initElements(driver, this);
	...
	{Thread.sleep(10000);
		...
		/*
		 * for (int z =0;z<x;z++) {int i=0; Thread.sleep(1000); card.get(z).click();
		 * //System.out.println(Heading.getText()); try { String text=Heading.getText();
		 * 
		 * str[i]=text; str[i] = Heading.getText(); System.out.println(str[i]); }
		 * catch(Exception e) { e.printStackTrace(); } }
		 */
