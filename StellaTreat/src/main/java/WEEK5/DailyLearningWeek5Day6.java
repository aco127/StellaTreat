...
public class DailyLearningWeek5Day6 {
	private static final String String = null;

	AndroidDriver driver;

	public DailyLearningWeek5Day6() {
		...
		/*List<WebElement> element = AppDriver.getDriver().findElements(By.className("android.widget.TextView"));
			...
			for (int j = 0; j < element.size(); j++) {
				if (element.get(j).getText() != null) {
					...
					/*
					 * try {
					 * //objExcelFile.writeExcel(System.getProperty("user.dir")+"\\TestReport",
					 * "Testdataa.xlsx","ExcelGuru99Demo",valueToWrite); } catch (IOException e) {
