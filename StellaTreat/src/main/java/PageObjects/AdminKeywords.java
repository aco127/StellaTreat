...
public void checkcontactpersonEmail(String emailid) throws InterruptedException
{
	try {
		...
		//emailinput.clear();
		Thread.sleep(2000);
		emailinput.sendKeys(emailcontactperson);
...
public void checkPatientEmail() throws InterruptedException
{
	...
	emailinput.sendKeys("onkar11@yopmail.com");
	go.click();
	Thread.sleep(3000);
	driver.switchTo().frame("ifmail");
	Thread.sleep(3000);
	System.out.println(subject.getText());
	Assert.assertEquals(subject.getText(), "Resources and Help from MamaLift", "Failed due to email");
}
...
public void checkNLPcontent() throws InterruptedException 
{
	...
	Thread.sleep(5000);
	//patientname.getText();
	WebElement name=driver.findElement(By.xpath("(//td[@role='gridcell'])[1]"));
	String names=name.getText();
	Assert.assertEquals(patientname.getText(), names, "Failed due to wrong patient name");
